...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('Send Report 📄')
    .addItem('Send TO only 📩', 'sendReportTOOnly')
    .addItem('Send CC only 📋', 'sendReportCCOnly')
...
function sendReportCCOnly() {
  sendReportWithFilter('CC');
}
...
function sendReportWithFilter(filterType) {
  ...
  try {
    ...
    recipientData.forEach(function(row) {
      ...
      if (filterType === 'CC' && recipientType !== 'CC') {
        skippedCount++;
        Logger.log('Skipped ' + recipientType + ' recipient: ' + recipientName);
        return;
      }
